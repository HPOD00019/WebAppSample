...
export const useChessNotationGame = (IssuerId: string, gameID: number, OnGameFinished: () => void, OnSetPlayBoardSide: (isWhite: boolean) => void, resetWhiteClock: (milliseconds: number) => void, resetBlackClock: (milliseconds: number) => void, wsConnectionString: string, fen?: string) : [string, () => string , (from: string, to: string) => boolean, (from: string, to: string, promotionType?: 'q'| 'n' | 'b' |'r') => boolean, (square: string) => string[], (from: string, to: string) => boolean] => {
    ...
    const [SendChessMessage] = useWSconnection(IssuerId, OnGameFinished,  OnSetPlayBoardSide ,OnChessMessageReceived, OnRefreshPosition, wsConnectionString);
